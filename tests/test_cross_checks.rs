...
mod crosschecks {
    ...
    fn test_fastlz_decompress_level_1() {
        ...
        let sys_size = sys::decompress(&input_comp, &mut sys_buff).unwrap_or(0);

        let mut native_buff = vec![0u8; orig_size + 100];
        let native_size = native::decompress(&input_comp, &mut native_buff).unwrap_or(0);

        assert_eq!(sys_size, native_size);
        assert_eq!(orig_size, native_size);
        assert!(compare_buffers(&sys_buff, &native_buff));
    }
